function Write-Color-LS
    {
        ...
        Write-host ("{0,-7} {1,25} {2,10} {3}" -f $file.mode, `
					([String]::Format("{0,10}  {1,8}", `
					$file.LastWriteTime.ToString("d"), `
					$file.LastWriteTime.ToString("t"))), `
					$file.length, $file.name) -foregroundcolor $color 
    }
...
New-CommandWrapper Out-Default -Process {
    ...
    $source_code = New-Object System.Text.RegularExpressions.Regex(
        '\.(vb|aspx|master|js|css|html|ascx|ashx|java|c|cpp|cs)$', $regex_opts)
    ...
    if(($_ -is [System.IO.DirectoryInfo]) -or ($_ -is [System.IO.FileInfo]))
    {
        ...
        elseif ($source_code.IsMatch($_.Name))
        {
