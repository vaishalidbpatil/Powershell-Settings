...
Move-Item "$env:UserProfile\Documents\WindowsPowerShell\Modules" "$env:UserProfile\Documents\WindowsPowerShell\Modules.Old.$((Get-Date).ToShortDateString().Replace('/','-'))" -ErrorAction SilentlyContinue 
...
Get-Blob -StorageAccount $StorageAccount -StorageKey $StorageKey | 
    ...
    ForEach-Object {
        ...
        Start-job -argumentList $moduleDir, $destModuleDir -Name "$($moduleDir.Name.Replace('-source', ''))" -ScriptBlock { 
            param($moduleDir, $destModuleDir) 
            $dir = Get-Item $destModuleDir
            dir $moduleDir -Recurse -Force |
