...
"@)        
        ...
        foreach ($parameter in $order) {
            ...
            $parameterIds += $parameterIdentifier
            if ($IsMandatory) {
...
"@)                    
    ...
    $null = $inputForm.Append("<input type='password' name='$inputFieldName' style='width:100%'>")                                        
                ...
                if ($pipeworksDirectives.Contains("Color") -and $Platform -eq 'Web') {
    ...
    <tr>
        <td style='text-align:right;margin-right:15px;padding:15px'>            
            ...
            <p style='text-align:center;margin-left:15px;padding:15px'>
                ...
                <input type='submit' class='$($commandMetadata.Name)_SubmitButton btn btn-primary' value='$buttonText' style='border:1px solid black;padding:5;font-size:large' $(if ($ajax) { "onclick='submit$($commandMetadata.Name.Replace("-", ''))();event.preventDefault();'" }) />
