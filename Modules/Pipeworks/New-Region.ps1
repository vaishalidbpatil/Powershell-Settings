...
"@
    ...
    process {
        ...
        if ($AsAccordian -or $AsTab -or $AsPopout -or $asPopin -or $AsPopdown -or $asPopup -or 
            ...
            $Asbutton -or $asTree -or $AsFeaturette -or $AsRow -or $AsColumn -or $AsHangingSpan -or $AsMenu) {
            $NotRequestAware = $true            
            $IsStaticRegion = $true       
            $DisableLayerSwitcher = $true     
            $FixedSize = $true            
...
<tr>
    <td style='text-indent: -21px;padding-left: 21px;$cssStyleAttr' colspan='4'>        
        ...
        <a id='$ShowButtonId' class='btn-primary' href='javascript:void(0)' style='$cssStyleAttr'>${LayerName}</a>
...
@"
    <script>
	    $(if ($pipeworksManifest.UseJQuery -or $pipeworksManifest.UseJQueryUI -or $pipeworksManifest.UseBootstrap) { @"
`$(function() {
        ...
        `$( "#$ShowButtonId" ).click( 
            function(){                            
                $ajaxLoad
                `$( "#$popoutLayerId" ).toggle(200);			                       
                `$( "#${popOutLayerId}_separator").toggle(200);
                `$( "#${popOutLayerId}_bottomseparator").toggle(200);
            })    		
	    });
"@} else { @"
        function ${ShowButtonId}_click() {
            $ajaxLoad
            var popout = document.getElementById('$popoutLayerId');
            var separator = document.getElementById('${popOutLayerId}_separator');
            var bottomseparator = document.getElementById('${popOutLayerId}_bottomseparator');
            if (popout.style.display == 'none') {
                popout.style.display = 'inline';
                separator.style.display = 'inline';
                bottomseparator.style.display = 'inline';
...
"@})
    ...
    <a id='$ShowButtonId'  href='$(if ($layerUrl[$layerName]) {} else { 'javascript:void(0)' })' style='padding-left:15px;padding-right:15px;padding-top:4%;padding-bottom:4%;margin-top:5px;margin-bottom:5px;$cssStyleAttr;text-align:center;display:block;float:left;' class='btn-primary span$SpanButtonSize' onclick='${ShowButtonId}_click();'>
        ${LayerName}
    </a>
...
<a id='$ShowButtonId'  href='javascript:void(0)' style='min-width:100%;text-decoration:underline;font-family:inherit;'><h1 style='$cssStyleAttr;'>${LayerName}</h1></a>
<div id='$popoutLayerId' class='' style="display:none;visibility:hidden;$cssStyleAttr">    
    $($layer[$layerName])
</div>
...
<a id='$ShowButtonId'  class='ui-widget-header ui-corner-all' $cssStyleChunk href='javascript:void(0)'>${LayerName}</a> 
"@                
...
<a id='$ShowButtonId'  class='ui-state-default ui-corner-all' href='javascript:void(0)' style='float:left;width:${GridItemWidth}px;height:${GridItemHeight}px;padding:2px;margin:5px;$cssFontAttr'>${LayerName}</a> 
"@                
...
<a id='$ShowButtonId' style='width:100%;$cssStyleAttr' class='ui-widget-header ui-corner-all' href='javascript:void(0)'>${LayerName}</a>
<br />
"@
            }
            ...
            if ($AsLeftSidebarMenu) {
                # Sidebar first
                $null = $outSb.Append("<div style='width:${sidebarWidth}%;text-align:center;float:left' valign='top'>$allbuttonContent</div>")
...
"@            
                ...
                if ($AsRightSidebarMenu) {
                    # Sidebar second
                    $null = $outsb.Append("<div style='width:${sidebarWidth}%;text-align:center;float:right'>$allbuttonContent</div>")                                     
                }
...
$SlidebuttonText= 
    if ($UseDotInsteadOfName) {
        "<a style='$cssFontAttr;text-decoration:none' id='$ShowButtonId' href='javascript:void(0)'><span style='font-size:6em'>&middot;</span></a>"
    } else {
        "<a style='padding:5px;$cssFontAttr' id='$ShowButtonId' href='javascript:void(0)'>${LayerName}</a>"
    }
